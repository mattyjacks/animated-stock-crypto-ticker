...
def generate_animation():
    ...
    ax.set_title(f'Stock Ticker: {ticker}', fontsize=16, color='white', pad=30)
    ...
    filename = f'stock-anim_{ticker}_{start_date}_{end_date}.mp4'
    ...
    # Update the UI
    status_label.config(text=f"Animation saved successfully as {filename}!")
...
skip_days_label.grid(row=3, column=0, padx=10, pady=10)
skip_days_entry = ttk.Entry(root)
skip_days_entry.insert(0, "0")  # Default value
skip_days_entry.grid(row=3, column=1, padx=10, pady=10)
...
include_start_check.grid(row=4, column=0, columnspan=2, pady=5)
...
include_end_check.grid(row=5, column=0, columnspan=2, pady=5)
...
custom_text_check.grid(row=6, column=0, columnspan=2, pady=5)
custom_text_box = Text(root, height=4, width=30)
custom_text_box.grid(row=7, column=0, columnspan=2, padx=10, pady=5)

# X-axis ticks interval
x_ticks_label = ttk.Label(root, text="X-axis Ticks Interval:")
x_ticks_label.grid(row=8, column=0, padx=10, pady=10)
x_ticks_entry = ttk.Entry(root)
x_ticks_entry.insert(0, "1")  # Default value for month intervals
x_ticks_entry.grid(row=8, column=1, padx=10, pady=10)

# Y-axis ticks interval
y_ticks_label = ttk.Label(root, text="Y-axis Ticks Interval:")
y_ticks_label.grid(row=9, column=0, padx=10, pady=10)
y_ticks_entry = ttk.Entry(root)
y_ticks_entry.insert(0, "10")  # Default value for price intervals
y_ticks_entry.grid(row=9, column=1, padx=10, pady=10)

# Chart height percentage entry
chart_height_label = ttk.Label(root, text="Chart Height Percentage:")
chart_height_label.grid(row=10, column=0, padx=10, pady=10)
chart_height_entry = ttk.Entry(root)
chart_height_entry.insert(0, "100")  # Default value of 100% height
chart_height_entry.grid(row=10, column=1, padx=10, pady=10)

# Generate button
generate_button = ttk.Button(root, text="Generate Animation", command=generate_animation)
generate_button.grid(row=11, column=0, columnspan=2, pady=20)
