...
def generate_animation():
    ...
    def update(frame):
        ...
        ax.set_xlabel('Date', color='white')
        ax.set_ylabel('Price', color='white')
        ax.text(0.5, 1.15, f"Start Date: {formatted_start_date}", ha='center', va='center', color='white', fontsize=12, transform=ax.transAxes)
        ax.text(0.5, 1.12, f"End Date: {formatted_end_date}", ha='center', va='center', color='white', fontsize=12, transform=ax.transAxes)
        
        # Display the current stock price next to the line
        current_price = data['Close'][frame]
        ax.text(data.index[frame], current_price, f"${current_price:.2f}", fontsize=14, ha='left', va='center', 
                bbox=dict(facecolor='yellow', edgecolor='white', boxstyle='round,pad=0.5'), color='black', fontweight='bold')
        
        # Set the x-axis date format to update less frequently
        ax.xaxis.set_major_locator(mdates.MonthLocator(interval=1))  # Update every month
...
end_entry = ttk.Entry(root)  # Correctly define end_entry here
